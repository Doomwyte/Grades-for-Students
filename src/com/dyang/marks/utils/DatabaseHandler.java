...
public class DatabaseHandler extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        String CREATE_COURSES_TABLE = "CREATE TABLE " + courses_table + " ("
                ...
                + KEY_CODE + " TEXT," + KEY_GPA + " TEXT)";
        String CREATE_CATEGORIES_TABLE = "CREATE TABLE " + categories_table
                + " (" + KEY_ID + " INTEGER PRIMARY KEY, " + KEY_NAME
                + " TEXT," + KEY_COURSE_ID + " INTEGER," + KEY_WEIGHT
                + " DOUBLE)";
        String CREATE_GRADES_TABLE = "CREATE TABLE " + grades_table + " ("
                + KEY_ID + " INTEGER PRIMARY KEY, " + KEY_GRADE_NAME + " TEXT,"
                + KEY_GRADE + " DOUBLE," + KEY_COURSE_ID + " INTEGER,"
    ...
    public void preAddCourse(int course_id) {
        ...
        db.delete(courses_table, KEY_ID + "=?",
                new String[] { Integer.toString(course_id) });
        db.close();
    }
    ...
    public void preAddCategory(int course_id) {
        SQLiteDatabase db = this.getWritableDatabase();
        db.delete(categories_table, KEY_COURSE_ID + "=?",
                new String[] { Integer.toString(course_id) });
        db.close();
    }
    ...
    public void preAddGrade(int course_id, int category_id) {
        ...
        db.delete(
                grades_table,
                KEY_COURSE_ID + "=? AND " + KEY_CATEGORY_ID + "=?",
                new String[] { Integer.toString(course_id),
    ...
    public int getNextGradeId() {
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor cursor = db.query(grades_table, new String[] { "MAX(" + KEY_ID
    ...
    public CourseObj getCourse(int id) {
        ...
        Cursor cursor = db.query(courses_table, new String[] { KEY_ID,
                KEY_NAME, KEY_CODE, KEY_GPA }, KEY_ID + "=?",
                new String[] { String.valueOf(id) }, null, null, null, null);
        if (cursor != null)
            cursor.moveToFirst();

        CourseObj course = new CourseObj(cursor.getInt(0), cursor.getString(1),
    ...
    // Getting all categories
    public List<CategoryObj> getAllEmptyCategories(int course_id) {
        List<CategoryObj> categoryList = new ArrayList<CategoryObj>();
        // Select All Query
        String selectQuery = "SELECT * FROM " + categories_table + " WHERE "
                + KEY_COURSE_ID + "=" + course_id;
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                // Checking grades count
                String gradeCheckQuery = "SELECT COUNT(*) FROM " + grades_table
                        + " WHERE " + KEY_CATEGORY_ID + "=" + cursor.getInt(0);
                Cursor gradeCursor = db.rawQuery(gradeCheckQuery, null);
                if (gradeCursor.moveToFirst() && gradeCursor.getInt(0) == 0) {
                    // Adding category to list
                    categoryList.add(category);
                }
            } while (cursor.moveToNext());
    ...
    public List<GradeObj> getAllGrades(int course_id, int category_id) {
        ...
        // Select All Query
        String selectQuery = "SELECT * FROM " + grades_table + " WHERE "
                + KEY_COURSE_ID + "=" + course_id + " AND " + KEY_CATEGORY_ID
                + "=" + category_id;
    ...
    public int getCategoriesCountByCourseId(int course_id) {
        String countQuery = "SELECT * FROM " + categories_table + " WHERE "
                + KEY_COURSE_ID + "=" + course_id;
    ...
    public int getGradesCount(int course_id, int category_id) {
        String countQuery = "SELECT * FROM " + grades_table + " WHERE "
                + KEY_COURSE_ID + "=" + course_id + " AND " + KEY_CATEGORY_ID
                + "=" + category_id;
    ...
    public void updateCourse(CourseObj course) {
        ...
        // updating row
        db.update(courses_table, values, KEY_ID + " = ?",
    ...
    public void deleteCourse(CourseObj course) {
        ...
        db.delete(categories_table, KEY_COURSE_ID + " = ?",
                new String[] { String.valueOf(course.getId()) });
        db.delete(grades_table, KEY_COURSE_ID + " = ?",
                new String[] { String.valueOf(course.getId()) });
        db.close();
    }
